...
def get_movement_vector(target_object):
    ...
    depth_component = 0
    if target_object.cx > ((ss.FRAME_PIXELS_X/2) + ss.FRAME_PIXELS_X*0.1):
        rotation_component = 1
        print("ROTATE RIGHT")
    elif target_object.cx < ((ss.FRAME_PIXELS_X/2) - ss.FRAME_PIXELS_X*0.1):
        rotation_component = -1
        print("ROTATE LEFT")
    else:
        print("DO NOT ROTATE")

    if target_object.cy > ((ss.FRAME_PIXELS_Y/2) + ss.FRAME_PIXELS_Y*0.1):
        depth_component = 1
        print("ASCEND")
    elif target_object.cy < ((ss.FRAME_PIXELS_Y/2) - ss.FRAME_PIXELS_Y*0.1):
        depth_component = -1
        print("DESCEND")
    else:
...
def investigate(target_info):
    ...
    while (target_object.bb_area < 0.8*ss.FRAME_AREA):
        get_movement_vector(target_object)
